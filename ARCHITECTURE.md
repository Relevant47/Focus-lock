...
macOS no longer has an `installer/` directory — the daemon ships inside
`FocusLock.app/Contents/Library/LaunchDaemons/` and is registered via
`SMAppService`. See "macOS install model" below.
...
The strip regex on both daemons also matches the legacy em-dashed form
(`# ── FocusLock START ──`) so older blocks self-heal on the next write.
