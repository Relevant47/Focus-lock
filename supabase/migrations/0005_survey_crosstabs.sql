-- 0005_survey_crosstabs — adds cross-cuts the dashboard needs for new charts.
-- Applied to project ipmmmebtsbhplcmwkflh via Supabase MCP.
--
-- Adds three new keys to the nightly snapshot:
--   responses_by_month — {"2026-01": 12, "2026-02": 24, ...} survey responses per month
--   os_by_month        — {"2026-01": {"mac": 8, "windows": 4}, ...} for stacked area chart
--   nps_by_age         — {"18-24": 7.3, "25-34": 8.1, ...} avg NPS per age bracket
...
returns void language sql security definer set search_path = public as $$
  ...
  values (current_date, now(), jsonb_build_object(
    ...
    'responses_by_month', (select coalesce(jsonb_object_agg(m,c),'{}') from (select to_char(created_at,'YYYY-MM') m, count(*) c from survey_responses group by 1 order by 1) t),
    'os_by_month',        (select coalesce(jsonb_object_agg(m, os_counts),'{}') from (
                              select m, jsonb_object_agg(os, c) os_counts
                              from (
                                select to_char(created_at,'YYYY-MM') m, primary_os::text os, count(*) c
                                from survey_responses
                                where primary_os is not null
                                group by 1, 2
                              ) per_month_os
                              group by m order by m
                           ) t),
    'nps_by_age',         (select coalesce(jsonb_object_agg(age_range, avg_nps),'{}') from (select age_range::text, round(avg(nps)::numeric,2) avg_nps from survey_responses where nps is not null and age_range is not null group by 1) t)
  ))
  on conflict (snapshot_date) do update set metrics = excluded.metrics, computed_at = excluded.computed_at;
$$;

-- Run once now so today's snapshot picks up the new keys without waiting for the 02:00 cron.
select public.refresh_survey_stats();
