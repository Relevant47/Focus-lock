-- 0002_survey_aggregates_cron — nightly aggregate snapshot for the dashboard.
...
returns void language sql security definer set search_path = public as $$
  ...
  values (current_date, now(), jsonb_build_object(
    ...
    'newsletter_optins',     (select count(*) from newsletter_optins),
    'newsletter_subscribed', (select count(*) from newsletter_optins where beehiiv_status='subscribed'),
...
select cron.schedule('refresh-survey-stats', '0 2 * * *', $$select public.refresh_survey_stats()$$);
